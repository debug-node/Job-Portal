...
export default function AdminUsers() {
    ...
    useEffect(() => {
        ...
        if (!key) router.push("/admin/login");
            ...
            setAdminKey(key);
            fetchUsers(key, 1, search);
        }
    }, []);

    const fetchUsers = async (key: string, pageNum: number, searchTerm: string) => {
        try {
            setLoading(true);
            ...
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_USER_SERVICE}/api/admin/users/all?${queryParams}`,
    ...
    const handleDelete = async (userId: number) => {
        ...
        try {
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_USER_SERVICE}/api/admin/users/delete/${userId}`,
                { 
                    method: "DELETE",
            ...
            fetchUsers(adminKey, page, search);
        } catch (error) {
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        <table className="w-full">
                            ...
                            <tbody>
                                {loading ? (
                                    ...
                                    users.map((user) => (
                                        <tr key={user.user_id} className="border-b dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-800/50">
                ...
                <div className="mt-6 flex justify-center gap-2">
                    <Button
                        onClick={() => {
                            setPage(page - 1);
                            fetchUsers(adminKey, page - 1, search);
                        }}
                        disabled={page === 1}
                        variant="outline"
                    >
